...
export const maxDuration = 600; // captcha solving (multi-round hCaptcha via 2Captcha) can take minutes
...
export async function POST(req: NextRequest) {
  if (req.method === 'POST') {
    try {
      ...
      const { prompt, tags, title, make_instrumental, model, wait_audio, negative_tags, persona_id } = body;
